...
function K_hamiltonian(Q, P, mu, C)
    ...
    V = (1/2)*(-1 + mu - Q[1]^2 + Q[2]^2 + Q[3]^2 - Q[4]^2) * (Q[2]*P[1] + Q[1]*P[2] - Q[4]*P[3] - Q[3]*P[4]) + (Q[1]*Q[2]-Q[3]*Q[4]) * (Q[1]*P[1] - Q[2]*P[2] - Q[3]*P[3] + Q[4]*P[4]) 

    # 正則化ハミルトニアン（r_M での除算なし）
    # K = r_M * (H + C/2) を展開して、r_M での除算を避ける形に変形
    K = (P[1]^2 + P[2]^2 + P[3]^2 + P[4]^2) / 8 + r_M * (-(1 - mu)/r_E + C/2) + V - mu
